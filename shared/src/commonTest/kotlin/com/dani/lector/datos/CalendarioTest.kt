...
class CalendarioTest {
    ...
    private fun aLas12(anio: Int, mes: Int, dia: Int): Long =
        LocalDateTime(anio, mes, dia, 12, 0)
            .toInstant(Novedades.ZONA).toEpochMilliseconds()
    ...
    @Test fun `las filas siempre son de siete`() {
        for (mes in 1..12) {
            ...
            // kotlinx-datetime no tiene lengthOfMonth: los dias del mes son
            // los que hay hasta el primero del mes siguiente.
            val primero = LocalDate(2026, mes, 1)
            val dias = primero.daysUntil(primero.plus(DatePeriod(months = 1)))
            assertEquals(dias, s.flatten().filterNotNull().size)
        }
    }
