...
object Novedades {
    ...
    fun aAvisar(
        ...
        for (n in numeros) {
            ...
            when {
                venta.isAfter(hoy) -> {}                // aun no ha salido: pendiente
                venta.isBefore(hoy.minusDays(CADUCIDAD)) -> callar.add(n)
                else -> avisar.add(n)
            }
        }
    ...
    fun venta(n: NumeroRemoto): LocalDate? =
        (fecha(n.venta) ?: fecha(n.fecha)?.minusDays(ADELANTO_PORTADA))
            ?.plusDays(DESFASE_ESPANA)
    ...
    fun enCristiano(cuando: LocalDate, hoy: LocalDate): String {
        ...
        val mes = meses.getOrElse(cuando.monthValue - 1) { "" }
     ...
     */
    fun agenda(
        fichas: List<SeriesRemotas.Ficha>,
        hoy: LocalDate,
        tope: Int = 15
    ): List<Prevista> =
    ...
    fun cuandoSale(cuando: LocalDate, hoy: LocalDate): String {
        val dias = java.time.temporal.ChronoUnit.DAYS.between(hoy, cuando)
        return when {
            dias <= 0L -> "ya"
            dias == 1L -> "mañana"
