...
object Calendario {
    ...
    fun clave(anio: Int, mes: Int, dia: Int) = "%04d-%02d-%02d".format(anio, mes, dia)

    /** La fecha española de un instante. Misma zona que todo lo demas. */
    fun fecha(cuando: Long): LocalDate =
        Instant.ofEpochMilli(cuando).atZone(Novedades.ZONA).toLocalDate()
    ...
    fun semanas(anio: Int, mes: Int): List<List<Int?>> {
        ...
        val dias = primero.lengthOfMonth()
